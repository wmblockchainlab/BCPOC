...
class Blockchain:
    ...
    def get_open_datas(self):
        """Returns a copy of the open datas list."""
        return self.__open_datas[:]
        ...
        try:
            with open('blockchain-{}.txt'.format(self.node_id), mode='r') as f:
                ...
                for block in blockchain:
                    ...
                    converted_de = [DataEntry(
                        de['entry_address'],
                        de['content'],
                        de['external_ids'],
                        de['signature']) for de in block['data_entry']]
                    updated_block = Block(
                        block['index'],
                        block['previous_hash'],
                        converted_tx,
                ...
                open_datas = json.loads(file_content[2][:-1])
                updated_datas = []
                for de in open_datas:
                    updated_data = DataEntry(
                        de['entry_address'],
                        de['content'],
                        de['external_ids'],
                        de['signature'])
                    updated_datas.append(updated_data)
                self.__open_datas = updated_datas
    ...
    def save_data(self):
        ...
        try:
            with open('blockchain-{}.txt'.format(self.node_id), mode='w') as f:
                saveable_chain = [
                    ...
                    [
                        Block(block_el.index,
                              ...
                              [de.__dict__ for de in block_el.data_entry],
                              block_el.proof,
                ...
                saveable_de = [de.__dict__ for de in self.__open_datas]
                f.write(json.dumps(saveable_de))
                f.write('\n')
    ...
    def add_data(self, entry_address, content, external_ids, signature):
        data_entry = DataEntry(entry_address, content, external_ids, signature)

        self.__open_datas.append(data_entry)
        self.save_data()
    ...
    def add_block(self, block):
        ...
        stored_datas = self.__open_datas[:]
        for ide in block['data_entry']:
            for opende in stored_datas:
                if (opende.entry_address == ide['entry_address'] and
                        opende.content == ide['content'] and
                        opende.external_ids == ide['external_ids'] and
                        opende.signature == ide['signature']):
                    try:
                        self.__open_datas.remove(opende)
                    except ValueError:
                        print('Item was already removed')
        self.save_data()
        return True
